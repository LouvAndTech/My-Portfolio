...
then
    ...
    docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t louvandtech/portfolio --push -f $DOCKERFILE .
elif [ $2 = '--active' ] || [ $2 = '-a' ];
then
    echo "Build for active Arch"
    #Build for active Arch
    docker build --tag louvandtech/portfolio -f $DOCKERFILE .
    docker push louvandtech/portfolio
else
